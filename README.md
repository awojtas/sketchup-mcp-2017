...
**Modelling** — `create_component`, `create_components` (many in one undo step), `array_copy` (repeat a finished part), `create_text` (raised lettering), `delete_component`, `transform_component`, `cut_pocket`, `solid_op`, `set_material`, `select`, `undo_last`
